function pc=plot_spot_number_v1(fp,frames,thresholds,NdSd,roi,aoiRadius,SequenceLength,ProbZ,frmave,fignum)
%
% function   plot_spot_number_v1(fp,frames,thresholds,NdSd,roi,aoiRadius,SequenceLength,Probz,frmave,fignum)
%
...
frmnum=length(frames);          % The number of different frames in which
                                % we detect spots
spots=spot_numbers_v1( fp,  frames,  thresholds,  NdSd, frmave, roi );
                                % Clear the present figure, then prepare
                                % to display a series of plots
figure(fignum);hold off;clf;hold on
for indx=1:frmnum
