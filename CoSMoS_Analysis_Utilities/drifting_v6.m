function pc=drifting_v6(gfolder,frmrange,frmlim,averange,fitwidth,filesource)
%
% function drifting_v6(gfolder,frmrange,frmlim,averange,fitwidth,filesource)
%
...
% v6  : make the two frames different sizes
% USE VERSION 5A:  WORKS AS WELL AS THIS ONE (AND SLIGHTLY FASTER)
pc=[];
mask=gaussian_spot(1,30,[0 0],1,53); % Make mask to zero frames at edges v6
...
[m n]=size(dum);
                                        ...
                                        % the grouped frames
                                        % the +6 is to allow unequal frame
                                        % sizes, version6
dum1=uint32(zeros(frmlim(2)-frmlim(1)+1+6,frmlim(4)-frmlim(3)+1+6,averange));               
dum2=dum1;                               
...
for frmindx=frmrange(1):frmrange(1)+averange-1
    fullframe=readaframe(gfolder,filesource,frmindx);
                                        % +-3 to allow unequal frame sizes,
                                        % version 6
    dum1(:,:,indx)=fullframe(frmlim(1)-3:frmlim(2)+3,frmlim(3)-3:frmlim(4)+3);
    indx=indx+1;
end
...
dum_prior_ave=dum_prior_ave-smooth_background(dum_prior_ave);
                    ...
                    % background
dum_prior_ave=dum_prior_ave.*mask;      % v6 push to zero at edges
[rosemm colnn]=size(dum_prior_ave);     % v6, so we can use diff size frames
autocorr=conv2(dum_prior_ave(4:rosemm-3,4:colnn-3),flipud(fliplr(dum_prior_ave)),'same');
...
for frmindx=(frmrange(1)+averange):averange:frmrange(2)
    ...
    for subfrmindx=frmindx:frmindx+averange-1
        fullframe=readaframe(gfolder,filesource,frmindx);
                                    % v6 +-3 to make diff size frames
        dum2(:,:,indx)=fullframe(frmlim(1)-3:frmlim(2)+3,frmlim(3)-3:frmlim(4)+3);
        indx=indx+1;
    end
    ...
    dum_current_ave=dum_current_ave.*mask;    % v6, push to zero at edges
    [rosemm colnn]=size(dum_current_ave);     % v6, so we can use diff size frames
    crosscor=conv2(dum_current_ave(4:rosemm-3,4:colnn-3),flipud(fliplr(dum_prior_ave)),'same');
