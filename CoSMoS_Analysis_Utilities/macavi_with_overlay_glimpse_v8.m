function pc=macavi_with_overlay_glimpse_v8(gfolder,frames,dispscale,bkgnd,frmrate,avifolder,frmave,driftlist,aoiinfo2a, aoiinfo2b,varargin)
%
% function macavi_with_overlay_glimpse_v8(gfolder,frames,dispscale,bkgnd,frmrate,avifolder,frmave,driftlist,aoiinfo2a,aoiinfo2b,<frmlim>)
%
...
% V6:  added capability of background subtraction  LJF 9/13/2014
% V7:  removed 'info' arguement from function
% V8:  replace movie2avi with VideoWriter for updated MatLab
...
green=[zeros(256,1) [0:255]'/255 zeros(256,1)];
mkavi='yes';
v = VideoWriter(avifolder);         %  movie2avi(pc,avifolder,'fps',frmrate,'quality',100,'compression','none');
v.Quality =100;
...
%        dum(:,:,rdindx-inum+1)=imread(imfold,'tif',rdindx);
    ...
    dum_display=imdivide(sum(dum,3) ,frmave);
     ...
     if bkgnd ==1
        ...
        dum_display=dum_display-bkgd_image(dum_display,5,2);
    end
...
dum_display=double(dum_display);
   ...
   figure(10);imagesc(rot90(dum_display,0),[lolim hilim]);colormap(cmap);axis equal;axis off
   ...
   %end
    for indx=1:max(aoiinfo2a(:,6))     % Cycle through all aois
       ...
       %draw_box(aoiinfo2a(indx,3:4)+XYshift,(aoiinfo2a(indx,5))/2,(aoiinfo2a(indx,5))/2,[.8 0 0]);
       draw_box(aoiinfo2a(indx,3:4)+XYshift,(aoiinfo2a(indx,5))/2,(aoiinfo2a(indx,5))/2,[1 .61 0]); % orange
      % draw_box(aoiinfo2a(indx,3:4)+XYshift,(aoiinfo2a(indx,5))/2,(aoiinfo2a(indx,5))/2,[1 1 0]); % yellow
    end
    hold on
    for indx=1:max(aoiinfo2b(:,6))     % Cycle through all aois
       ...
       %draw_box(aoiinfo2b(indx,3:4)+XYshift,(pixnum)/2,(pixnum)/2,[0 0 1]);
       draw_box(aoiinfo2b(indx,3:4)+XYshift,(aoiinfo2b(indx,5))/2,(aoiinfo2b(indx,5))/2,[0 0 1]);
    end
   hold off
