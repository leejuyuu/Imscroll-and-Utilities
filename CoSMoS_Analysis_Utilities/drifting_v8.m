function pc=drifting_v8(gfolder,frmrange,frmlim,averange,fitwidth,filesource,folder)
%
% function drifting_v7(gfolder,frmrange,frmlim,averange,fitwidth,filesource,folder)
%
...
% v8: allow reading tiff files as well
pc=[];
...
indx=1;
if filesource==2
                % here to read tiff frames
    for frmindx=frmrange(1):frmrange(1)+averange-1
        fullframe=imread(folder,'tif',frmindx);
    ...
    if filesource==2
        ...
        for subfrmindx=frmindx:frmindx+averange-1
           ...
           indx=indx+1;
                ...
                % Here to read from glimpse folder
        for subfrmindx=frmindx:frmindx+averange-1
            fullframe=readaframe(gfolder,filesource,frmindx);
            dum2(:,:,indx)=fullframe(frmlim(1):frmlim(2),frmlim(3):frmlim(4));
            indx=indx+1;
