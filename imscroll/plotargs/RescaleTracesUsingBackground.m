...
%equation  A: (Refaoifits-Bkgndaoifits)/ ( (Bkgndaoifits-offsetaoifits)/(Bkdgndaoifits-offsetaoifits)N)
% Protocol of function 
% (1) Run the SmoothBackground_v2 w/ Refaoifits and Bkgndaoifits?Subaoifits
% (2) average all the smoothed offset aoifits to obtain a single trace for the smoothed offset trace (zero excitation trace)
% (3) Calculate set of values for and the distribution of the median for (Bkgndaoifits-Offsetaoifits) traces
% (4) Pick N = ref AOI # for median (Bkgndaoifits – offsetaoifits)N
% (5) Calculated corrected traces using eqn A above
...
pc.Bkdata=Subaoifits.Bkdata;      % Contains the smoothed averaged background data
